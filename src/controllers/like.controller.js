...
const toggleVideoLike = asyncHandler(async (req, res) => {
    ...
    if (existingLike) {
        ...
        if (!deleteExistingLike) {
            throw new ApiError(
                500,
                "Something went wrong while removing the like on the video"
            )
        }
        ...
        return res
            .status(200)
            .json(new ApiResponse(200, newLike, "The video is liked successfully"))
    }
    ...
    if(tweetLike){
        ...
        .json(
            new ApiResponse(
                200,
                deleteTweetLike,
                "Deleted the tweet like successfully"
            )
        )
    }
    if(!tweetLike){
        ...
        return res
        .status(200)
        .json(new ApiResponse(200, likeTheTweet, "Liked the tweet successfully"))
    }
